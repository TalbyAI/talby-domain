...
`P` es entero positivo, `S` entero no negativo; en este perfil ambos tienen techo 4096 y, si ambos se declaran, `S≤P`. Omitir cualquiera no introduce una restricción de negocio para ese parámetro. Al acumular límites se utiliza el más restrictivo; un límite de escala mayor que la precisión efectiva es redundante y puede reducirse a ella, sin rechazar una composición que siga siendo satisfacible.
...
6. `compareDecimal(decimal("0.1"),decimal("0.10"))=0`; comparación con valores superiores al entero seguro; `compareDate(fin,inicio)>=0`; offsets diferentes que expresan el mismo instante.
